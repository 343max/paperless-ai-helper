...
import { processPaperlessDocument } from "./processPaperlessDocument.ts"
...
const main = async () => {
  try {
    ...
    for (const documentId of documentIds) {
      await processPaperlessDocument(documentId)
    }
  } catch (error) {
    console.error(error)
  }
...
app.post("/process/:documentId", async (req, res) => {
  ...
  try {
    await processPaperlessDocument(documentId, true)
